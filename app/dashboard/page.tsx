  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Car} label="المركبات المسجلة" value={stats.totalVehicles} color="bg-purple-500" />
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-4 mb-3">
          <span className="text-sm text-gray-500">الإشغال</span>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="text-center p-3 bg-green-50 rounded-xl">
            ...
            <p className="text-xs text-green-600 mt-1">متاح</p>
          </div>
          <div className="text-center p-3 bg-red-50 rounded-xl">
            <p className="text-2xl font-bold text-red-600">{stats.occupiedSlots}</p>
            <p className="text-xs text-red-600 mt-1">مشغول</p>
          </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
              <tr>
                ...
                <th className="text-right px-6 py-3">الوقت</th>
                <th className="text-right px-6 py-3">الحالة</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {recentReservations.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-8 text-gray-400">لا توجد حجوزات بعد</td></tr>
              ) : recentReservations.map(r => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 text-sm font-medium text-gray-900">{r.user.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{r.vehicle.plateNumber}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{r.slot.zone.name} - {r.slot.slotNumber}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{new Date(r.startTime).toLocaleDateString('ar-SA')}</td>
